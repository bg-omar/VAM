...
one can estimate the needed vorticity field for a desired $\Delta P$. For example, achieving a 1% weight reduction ($\epsilon=0.01$) on a small test mass (height $h\sim0.1$ m, $\rho_{\rm obj}\sim 10^3\text{kg/m}^3$) in Earth’s field ($g=9.8$) requires $\Delta P\sim0.01 \times \rho_{\rm obj} g h \approx 9.8\times10^1~\text{Pa} = O(10^2~\text{Pa})$. Equation \eqref{eq:dp} shows that to produce $\sim100$ Pa via æther vorticity, given $\rho_{\æ}$ as small as $10^{-6}$ to $10^{-5}$ kg/m³ near Earth’s surface, one needs $(\nabla|\omega|^2)$ on the order of $10^8$ to $10^9~\text{s}^{-2}$ per meter. This is achievable with very intense localized vortices (for instance, $\omega \sim 10^4$–$10^5~\text{s}^{-1}$ varying over millimeter scales). Rotating superconductors and magnetic fields can create such conditions: experiments have indeed shown that a rapidly spinning superconducting disk in a magnetic field can produce small weight reductions in objects above it. Follow-up measurements indicate measurable lift (weight loss 0.1–2%) correlating with the disk’s rotation speed and magnetic field, consistent with vorticity-induced pressure gradients. In our VAM interpretation, the superconductor’s rotating magnetic field drags the local æther into a vortex, partially shielding gravity by superposing a new $\Phi_v$ opposite to Earth’s.
...
\frac{t_{\rm local}}{t_{\rm background}} \;=\; \Big(1 + \tfrac{1}{2}\,\beta\,I\,\Omega_k^2\Big)^{-1}, \tag{3}\label{eq:timedil}

...
\begin{description}
    ...
    The æther behaves as an ideal fluid with zero viscosity and approximately constant density $\rho_\text{\ae}$ in large-scale regions (analogous to superfluid helium at $T=0$). Here, $\rho_\text{\ae}$ represents the energy density of the æther (units J/m$^3$), not the classical mass density. In geometric units, this is equivalent to kg/m$^3$. However, compressibility is permitted near vortex cores and in regions of high swirl energy, where density gradients support field dynamics and mass-energy emergence. This scale-dependent compressibility is negligible for low-energy vortical flow but crucial in quantum-scale interactions.
    ...
    Spatial gradients in vorticity or helicity (twisting of vortex lines) in the æther field can influence other vortices. For example, what we observe as a $\text{"gravitational field"}$ will be modeled by a certain æther velocity field (as we will explain later). The principle of maximum force $ F^{\text{max}}_{\text{gr}} = c^4 / 4 G $ from general relativity~\cite{Schiller2022-maxforce}, which sets an upper limit on force in nature, is assumed to arise from properties of the æther (e.g., maximum flow velocity $c$ and density $\rho_\text{\ae}$ impose a limit on momentum flux/force).
\end{description}
...
\begin{equation*}
    F^{\text{max}}_{\text{gr}} = \frac{c^4}{4G} \approx 3.0\times10^{43}\ \text{N}.
\end{equation*}
...
We retain $c$ (speed of light in vacuum) as the characteristic signal speed in the æther (e.g., the speed of sound or wave propagation in the superfluid vacuum, often taken as $c = \sqrt{B/\rho_\text{\ae}}$\footnote{If interpreted as a compressional wave speed, this assumes an effective bulk modulus $B$ and a background energy density $\rho_\text{\ae}$. However, in vortex-dominated regions, the dynamics are better captured by circulation and tension, not compressive elasticity.} for bulk modulus $B$). The Newtonian gravitational constant $G$ will enter when linking æther flow to mass (since mass is essentially a vortex with a certain circulation and core structure that relates to $G$). We will introduce any additional constants as needed.
...
dil = np.sqrt(np.maximum(0, 1 - rs * 2 / r))
inset_ax.plot(r, dil, color='purple')
inset_ax.set_title(r'$\frac{d\tau}{dt}$ vs $r$', fontsize=11)
inset_ax.set_xlabel(r'$r$', fontsize=11)
inset_ax.set_ylabel(r'$\frac{d\tau}{dt}$', fontsize=11)
inset_ax.grid(True)
...
ax.set_xlim(-6, 6)
ax.set_ylim(-6, 6)
ax.set_title("Time freeze at event horizon", fontsize=14)
...
ax.add_patch(vortex)
ax.plot(x_vortex + 0.4, y_vortex, 'ro', markersize=10)
ax.text(x_vortex, y_vortex + 0.6, 'Frozen vortex', ha='center', fontsize=11)

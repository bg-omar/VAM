...
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(v_rel, dil, color='darkorange', linewidth=2)
ax.set_title(r"Unified time dilation:: $\frac{d\tau}{dt} = \sqrt{1 - \frac{|\vec{v}_{\mathrm{rel}}|^2}{c^2}}$", fontsize=14)
ax.set_xlabel(r"$|\vec{v}_{\mathrm{rel}}|$ (relative speed to æther)", fontsize=12)
ax.set_ylabel(r"$\frac{d\tau}{dt}$ (clock speed)", fontsize=12)
ax.grid(True)
ax.set_xlim(0, 1)
ax.set_ylim(0, 1.05)

# Annotaties
ax.annotate("Rest (v = 0)", xy=(0, 1), xytext=(0.1, 0.95),
            arrowprops=dict(arrowstyle="->", color='black'), fontsize=11)
ax.annotate("Circular orbit (v ≈ √3GM/r)", xy=(0.6, np.sqrt(1 - 0.6**2)),
            xytext=(0.7, 0.65),
            arrowprops=dict(arrowstyle="->", color='black'), fontsize=11)
ax.annotate("Horizon (v = c)", xy=(0.99, 0), xytext=(0.75, 0.2),
            arrowprops=dict(arrowstyle="->", color='black'), fontsize=11)

...
def vam_mass_core(nucleons, electrons, beta=beta_default, kappa=1.0, m_threads = 1, eq=0):
...
# Merge orbital radii into main DataFrame
df_radii = pd.DataFrame(radii, columns=["Name", "R_x (m)"])  # Rename to match for merge
df = pd.DataFrame(rows, columns=["Name", "VAM Mass (kg)", "Actual Mass (kg)", "% Difference"])

# Merge on 'Name'
df = pd.merge(df, df_radii, on="Name", how="left")

# Display combined table
df.index = df.index + 1  # Make index start at 1
tools.display_dataframe_to_user(name="VAM vs Actual Mass with Orbital Radius", dataframe=df)
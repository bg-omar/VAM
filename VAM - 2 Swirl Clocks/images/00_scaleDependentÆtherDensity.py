...
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_xlabel('Radial distance r (m)', fontsize=12)
ax.set_ylabel('Ætherdensity ρₐ (kg/m³)', fontsize=12)
ax.set_title('Scale-dependent Ætherdensity in VAM $ \\rho_\\text{\\ae} (r) = \\rho_\\text{far} + (\\rho_\\text{core} - \\rho_\\text{far}) e^{-r / r_*} $', fontsize=14)
ax.grid(True, which='both', linestyle='--', linewidth=0.5)
...
inset_ax.set_xscale('log')
inset_ax.set_yscale('log')
inset_ax.set_title('Zoom: Dense core', fontsize=10)
inset_ax.set_xlabel('r (m)', fontsize=8)
inset_ax.set_ylabel('ρₐ (kg/m³)', fontsize=8)
inset_ax.tick_params(axis='both', which='major', labelsize=8)
inset_ax.grid(True, which='both', linestyle=':', linewidth=0.5)

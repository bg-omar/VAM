...
plt.figure(figsize=(10, 6))
plt.plot(r_zoom, t_local_r_zoom, color='darkgreen')
plt.title("Radiaal profiel van lokale tijddilatatie met Ωₖ ∝ 1/r²")
...
# ✅ Get the script filename dynamically
script_name = os.path.splitext(os.path.basename(__file__))[0]
filename = f"{script_name}_Radial_LocalTime_Dilation_nl.png"
plt.savefig(filename, dpi=150)  # Save image with high resolution
...
contour = plt.contourf(X * 1e15, Y * 1e15, t_local_2D, levels=50, cmap='viridis')
cbar = plt.colorbar(contour)
cbar.set_label("Lokale tijd $t_{lokaal} / t_{abs}$")
plt.title("Vortex-geïnduceerde tijdsput (femtometerschaal)")
plt.xlabel("x [fm]")
plt.ylabel("y [fm]")
plt.axis('equal')
plt.grid(True)
...
filename = f"{script_name}_Vortex-Induced_Time_Well_nl.png"
plt.savefig(filename, dpi=150)  # Save image with high resolution

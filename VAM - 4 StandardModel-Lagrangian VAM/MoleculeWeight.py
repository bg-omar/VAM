...
def vam_mass_core(nucleons, electrons, beta=beta_default, kappa=1.0, m_threads = 1, eq=2):
    ...
    if eq == 2: # 1 → tan(φ)-based swirl suppression
        # Coherence Correction Factor ξ(n)
    ...
    # # Amplification
    # # Thread Suppression Factor η(m)
    # # Assume all nucleons are cabled: m = 2
    # # Assume electrons are torus knots: m = 1
    # if nucleons > 0 and electrons > 0:
    #     eta_nucleons = (1 / 2) ** 1.5
    #     eta_electrons = 1.0
    #     total_knots = nucleons + electrons
    #     # Thread Suppression Factor η(m)
    #     eta = ((eta_nucleons * nucleons + eta_electrons * electrons) / total_knots)**1.5
    # else:
    #     eta = (1 / m_threads) ** 1.5
...
# ─── Vortex Creation ───────────────────────────────────────────────────────────
def make_vortices(p, n):
    total_knots = p + n
    R_x = compute_orbital_radius(n, p + n)  # approximate orbital radius
    r_k = r_c
    V_k = 2 * math.pi**2 * R_x * r_k**2
    omega2 = (C_e / r_k)**2

    vortices = []
    for idx in range(total_knots):
        vortices.append({
            'volume': V_k,
            'rho': rho_core,
            'omega_sq_mean': omega2,
            'helicity': 0.0,
            'torsion': 0.0,
            'position': np.array([idx*2*r_k, 0.0, 0.0])
        })
    return vortices
...
for name, p, n, e, gmol in atoms_molecules:
    ...
    # Vortex integral
    if p + n < 300:
        vort = make_vortices(p, n)
        int_mass = vam_mass_integral(vort)
        diff_int = 100 * (int_mass - m_act) / m_act
    else:
        int_mass = float('nan')
        diff_int = float('nan')

...
fig, axes = plt.subplots(1, 2, figsize=(12, 6))
titles = ['(1) Particle at rest in æther', '(2) Particle in motion with speed v']
omega_labels = [r'$\omega_0$', r'$\omega_{\mathrm{obs}} < \omega_0$']
...
for i, ax in enumerate(axes):
    ...
    if i == 1:
        ...
        ax.text(0, 1.1, r'Æthercurrent $\vec{v}$ around vortex', fontsize=12, ha='center', color='navy')
        # ax.text(1.3, 1.6, r'$|\vec{v}| = v$', fontsize=11, color='navy')
...
plt.suptitle("Effect of æthermotion on rotation of vortex core", fontsize=14)
plt.tight_layout()
import os
script_name = os.path.splitext(os.path.basename(__file__))[0]

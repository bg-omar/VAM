...
# Focused plot on the heuristic only
plt.figure(figsize=(8, 5))
plt.plot(r_zoom, term_heuristic_zoom, color='green', label="Heuristische tijdsverhouding")
plt.axvline(r_c, linestyle='--', color='gray', label=r"$r = r_c$")

plt.xscale('log')
plt.ylim(0.45, 1.01)
plt.xlim(1e-15, 1e-14)
plt.xlabel("Straal $r$ [m] (logschaal, nabij $r_c$)")
plt.ylabel(r"Tijdsverhouding $t_\text{lokaal} / t_\text{abs}$")
plt.title("Ingezoomde weergave: Heuristische tijdsdilatatie nabij wervelkern")
plt.legend()
plt.grid(True)
plt.tight_layout()
import os
script_name = os.path.splitext(os.path.basename(__file__))[0]
filename = f"{script_name}4_nl.png"
plt.savefig(filename, dpi=150)  # Save image with high resolution
plt.show()

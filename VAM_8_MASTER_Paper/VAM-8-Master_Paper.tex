...
% --- helpers for inline knot icons (keeps them centered in math mode) ---
                \newcommand{\knicon}[1]{\vcenter{\hbox{\includegraphics[height=1.2em]{#1}}}}
...
% ----------------------------- VAM Used Knots -----------------------------
                ...
                \[
                    ...
                    \begin{aligned}
                        ...
                        \knlab{Hopf link}{images/hopf}:&
                        \ \ g~\text{(gluon-like flux / gauge carrier; massless)}
                        && \text{(not a ``massive polariton''); }\cite{VAM6}. \\[6pt]

                        \knlab{Trefoil}~T(2,3)~{\knicon{images/3_1}}:&
                        \ \ e^- \quad \text{(mirror }T(3,2)\text{: }e^+\text{)}
                        && \text{(chiral fermion; spin-$\tfrac12$); }\cite{VAM5,VAM11-leptons}. \\[4pt]

                        \knlab{$5_2$}{images/5_2}:&
                        \ \ u\text{-type subknot (chiral)}
                        && \text{fermion (spin-$\tfrac12$); }\cite{VAM5,VAM11-leptons}. \\[4pt]

                        \knlab{$6_1$}{images/6_1}:&
                        \ \ d\text{-type subknot (chiral)}
                        && \text{fermion (spin-$\tfrac12$); }\cite{VAM5-quarks}. \\[4pt]

% If you have an image for the fully pairwise-linked triple, put its filename below.
% Temporarily using a placeholder name; replace `images/triplelink` with your file if different.
                        \knlab{Triple link (all pairs linked)}{images/aborromean}:&
                        \ \ p~(uud;\ 5_2,\,5_2,\,6_1)
                        && \text{(not Borromean); }\cite{VAM5-protneut}. \\[4pt]

                        \knlab{Borromean rings}{images/borromean}:&
                        \ \ n~(udd;\ 5_2,\,6_1,\,6_1)
                        && \text{(pairwise $L_k=0$, inseparable triple); }\cite{VAM5-protneut}. \\[4pt]

% Using Solomon image as the visual for a two-component Hopfion-like doublet
                        \knlab{Hopfion doublet}{images/solomon}:&
                        \ \ \nu~\text{(neutrino; linked, neutral swirl mode)}
                        && \text{alt.: near-unknot minimal-tension state; }\cite{VAM5-nu,VAM11-nu}. \\[4pt]

                        \knlab{Figure-eight}{images/4_1}:&
                        \ \ \text{achiral hyperbolic; \emph{expelled} from chiral swirl fields}
                        && \text{(DE-like repulsion, \emph{not} DM); }\cite{VAM9}.
                    \end{aligned}
                    \endgroup
                \]

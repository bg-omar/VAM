#!/usr/bin/env python3
import os, sys, math, glob, argparse, csv
import numpy as np
import pandas as pd
...
def main():
    ap = argparse.ArgumentParser(description="Recursive *.fseries → VAM invariants and masses")
    ap.add_argument("--dir", type=str, default=".", help="Root directory to scan recursively for *.fseries")
    ...
    ap.add_argument("--cr_dirs", type=int, default=28, help="Random directions for crossing estimate")
    ...
    ap.add_argument("--emit_meta", type=str, default="", help="If set, write a meta skeleton CSV for discovered knot_ids")
    args = ap.parse_args()

    # Recursively find *.fseries
    pattern = os.path.join(args.dir, "**", "*.fseries")
    files = sorted(glob.glob(pattern, recursive=True))
    if not files:
        print("No .fseries files found under", args.dir)
        return

    # Optionally emit meta skeleton
    if args.emit_meta:
        ids = discover_ids(files)
        with open(args.emit_meta, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["knot_id","chiral","sigma","type","hyperbolic_volume"])
            for kid in ids:
                w.writerow([kid, "", "", "", ""])  # fill in later

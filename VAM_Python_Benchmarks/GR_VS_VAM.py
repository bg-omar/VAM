...
df.to_csv(f"{script_name}.csv", index=False)
df.to_excel(f"{script_name}.xlsx", index=False, engine="openpyxl")
print("\nBenchmark data saved to ", f"{script_name}.csv")

df["|GR - VAM| dτ/dt"] = abs(df["GR dτ/dt"] - df["VAM dτ/dt"])
df["|GR - Obs| dτ/dt"] = abs(df["GR dτ/dt"] - df["Obs dτ/dt"])


def ansi_color(val1, val2, tol=1e-3):
    if pd.isnull(val1) or pd.isnull(val2):
        return f"{val2:.3e}" if val2 is not None else "N/A"
    diff = abs(val1 - val2)
    return f"\033[92m{val2:.3e}\033[0m" if diff <= tol else f"\033[91m{val2:.3e}\033[0m"

print("\n\033[1m=== GR vs VAM dτ/dt Comparison ===\033[0m")
for row in results:
    gr = row.get("GR dτ/dt")
    vam = row.get("VAM dτ/dt")
    obs = row.get("Obs dτ/dt")
    vam_str = ansi_color(gr, vam)
    obs_str = ansi_color(gr, obs)
    print(f"{row['Object']:<15} GR: {gr:.3e} | VAM: {vam_str} | Obs: {obs_str}")

import matplotlib.pyplot as plt

objects = [row["Object"] for row in results]
gr_vals = [row["GR dτ/dt"] for row in results]
vam_vals = [row["VAM dτ/dt"] for row in results]
obs_vals = [row["Obs dτ/dt"] for row in results]

plt.figure(figsize=(10, 6))
plt.plot(objects, gr_vals, marker='o', label='GR dτ/dt')
plt.plot(objects, vam_vals, marker='x', label='VAM dτ/dt')
plt.plot(objects, obs_vals, marker='^', label='Obs dτ/dt')
plt.yscale('log')
plt.ylabel('Time Dilation (dτ/dt)')
plt.title('GR vs VAM vs Observed Time Dilation')
plt.legend()
plt.grid(True)
plt.tight_layout()

...
# base_sequence = [int(1 + ((0.5 * coil_corners) - 2) % coil_corners) for _ in range(coil_corners)]
base_sequence = [(1 + int(next_corner)* i) % coil_corners + 1 for i in range(coil_corners + 1)]
# base_sequence = [(1 + 5 * i) % coil_corners + 1 for i in range(coil_corners)]

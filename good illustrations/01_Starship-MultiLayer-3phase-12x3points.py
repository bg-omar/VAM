...
def generate_3d_wire(sequence, segment, z_layer, base_color, style, alpha=1.0):
    ...
    angles = np.linspace(0, 2 * np.pi, (total_corners)+1)[:-1] - np.pi * 1.5 + (2 * np.pi * (1 / (total_corners)))
    angles = angles[::-1]  # Reverse numbering for clockwise count
    segment_shift = ((2 * np.pi / (total_corners)) / coil_amount)
    ...
    for i in range(len(sequence) - 1):
        ...
        angle_start = angles[(num - 1) % (total_corners)] + segment_shift * (segment - 1)
        angle_end = angles[(next_num - 1) % (total_corners)] + segment_shift * (segment - 1)

...
# === Generate knot and tangents ===
positions = generate_figure8_knot()
tangents = compute_tangents(positions)

# === 3D grid for field evaluation ===
grid_N = 15
coords = np.linspace(-4, 4, grid_N)
Xg, Yg, Zg = np.meshgrid(coords, coords, coords, indexing='ij')

# === Compute velocity V on grid ===
V = np.zeros((grid_N, grid_N, grid_N, 3))
for i in range(grid_N):
    for j in range(grid_N):
        for k in range(grid_N):
            r = np.array([Xg[i,j,k], Yg[i,j,k], Zg[i,j,k]])
            V[i,j,k] = biot_savart_velocity(r, positions, tangents)

# === Compute vorticity (curl of V) ===
dx = coords
dy = coords
dz = coords
Vx = V[...,0]; Vy = V[...,1]; Vz = V[...,2]
dVy_dz, dVy_dy, dVy_dx = np.gradient(Vy, dz, dy, dx, edge_order=2)
dVz_dz, dVz_dy, dVz_dx = np.gradient(Vz, dz, dy, dx, edge_order=2)
dVx_dz, dVx_dy, dVx_dx = np.gradient(Vx, dz, dy, dx, edge_order=2)
...
# Plot vorticity shell (points above 90th percentile)
threshold = np.percentile(omega_mag, 90)
mask = omega_mag >= threshold
ax.scatter(
    Xg[mask], Yg[mask], Zg[mask],
    c=omega_mag[mask], cmap='inferno', s=20, alpha=0.8,
    label='Vorticity Shell'
)

# Quiver velocity (sparse sampling)
step = 3
ax.quiver(
    Xg[::step,::step,::step], Yg[::step,::step,::step], Zg[::step,::step,::step],
    V[::step,::step,::step,0], V[::step,::step,::step,1], V[::step,::step,::step,2],
    length=0.5, normalize=True, color='gray', alpha=0.4
)

# Plot the knot filament
ax.plot(
...
# Formatting
ax.set_title('3D Vorticity Shell + Velocity Field around Figure-8 Knot')

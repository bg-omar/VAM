...
helix_x = (R + r * np.cos(3 * helix_theta)) * np.cos(helix_theta)
helix_y = (R + r * np.cos(3 * helix_theta)) * np.sin(helix_theta)
helix_z = r * np.sin(3 * helix_theta)
...
ax.set_ylabel("Y")
ax.set_zlabel("Z")
ax.set_title("3 Helical Spirals")
...
helix_y3 = (R + r * np.cos(3 * helix_theta + 4 * np.pi / 3)) * np.sin(helix_theta + 4 * np.pi / 3)
helix_z3 = r * np.sin(3 * helix_theta + 4 * np.pi / 3)
ax.plot(helix_x, helix_y, helix_z, 'r-', linewidth=1)
ax.plot(helix_x2, helix_y2, helix_z2, 'b-', linewidth=2)
#ax.plot(helix_x3, helix_y3, helix_z3, 'g-', linewidth=2)
...
for i, (helix_x, helix_y, helix_z) in enumerate(helices):
    color = ['r-', 'b-', 'g-'][i % 3]
    ax.plot(helix_x, helix_y, helix_z, color, linewidth=1, alpha=0.4)

# Identify points where the helix reaches the outer edge
num_labels = 36  # Adjust for different numbers (3, 6, 9, etc.)
for i in range(num_labels):
    index = i * len(helix_theta) // num_labels
    ax.text(4.1*np.cos(helix_theta)[index], 4.1*np.sin(helix_theta)[index], 0, str(i + 1), color='black', fontsize=14, ha='center', va='center')
# Plot helical spiral through the center hole
...
# Define additional helices for positions 2-5-8 and 3-6-9
helix_x2 = (R + r * np.cos(helix_phi+ 2 * np.pi / 3)) * np.cos(helix_theta + 2 * np.pi / 3)
helix_y2 = (R + r * np.cos(helix_phi+ 2 * np.pi / 3)) * np.sin(helix_theta + 2 * np.pi / 3)
helix_z2 = r * np.sin(helix_phi)

helix_x3 = (R + r * np.cos(helix_phi + 4 * np.pi / 3)) * np.cos(helix_theta + 4 * np.pi / 3)
helix_y3 = (R + r * np.cos(helix_phi + 4 * np.pi / 3)) * np.sin(helix_theta + 4 * np.pi / 3)
helix_z3 = r * np.sin(helix_phi)
ax.plot(helix_x, helix_y, helix_z, 'r-', linewidth=2)
ax.plot(helix_x2, helix_y2, helix_z2, 'b-', linewidth=2)
ax.plot(helix_x3, helix_y3, helix_z3, 'g-', linewidth=2)
# Create multiple subplots for different views
